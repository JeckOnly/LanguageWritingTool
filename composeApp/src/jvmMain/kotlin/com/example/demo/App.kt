...
fun App(vm: MainViewModel) {
    ...
    EnglishFixerTheme(darkTheme = s.isDarkTheme) {
        // ✅ 关键：用 Surface 把整个窗口背景真正绘制成 theme 的 background
        Surface(
            ...
            Column(
                ...
                Row(
                    ...
                    // ✅ 主题切换：一般不需要跟 loading 绑定（你想切就切）
                    OutlinedButton(onClick = vm::toggleTheme) {
                        Text(if (s.isDarkTheme) "Light" else "Dark")
                    }

                    Button(onClick = vm::checkNow, enabled = !s.isLoading) {
                        Text("Check (⌘Enter)")
                    }

                    OutlinedButton(onClick = vm::cancel, enabled = s.isLoading) {
                        Text("Cancel (Esc)")
                    }
                }
